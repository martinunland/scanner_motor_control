...
class Motor:
    def __init__(self, port, motor_number, baudrate, dist_per_rot):
    ...
    def _microsteps_to_distance(self, position):
        micsteps_per_rot = 200 * (2 ** self._get_parameter(TMCLPars.MICROSTEP_RESOLUTION))
        distance = position * self.dist_per_rot / micsteps_per_rot
        return distance

    def _distance_to_microsteps(self, distance):
        micsteps_per_rot = 200 * (2 ** self._get_parameter(TMCLPars.MICROSTEP_RESOLUTION))
    ...
    def move_in_step(self, steps, mode: MotorMovement):
        """
    ...
    def move_relative_distance_in_mm(self, distance_mm):
        """
        ...
        """
        self.move_in_step(self._distance_to_microsteps(distance_mm), mode=MotorMovement.RELATIVE)

    def move_absolute_position_in_mm(self, position_mm):
        """
        ...
        """
        self.move_in_step(self._distance_to_microsteps(position_mm), mode=MotorMovement.ABSOLUTE)
    ...
    def search_reference_position(self):
        ...
        self._write_to_serial(TMCL.set_axis_parameter(TMCLPars.ACTUAL_POSITION_MICROSTEPS, 0))
